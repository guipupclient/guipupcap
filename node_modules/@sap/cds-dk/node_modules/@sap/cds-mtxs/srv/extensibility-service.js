...
module.exports = class ExtensibilityService extends cds.ApplicationService {
  async init() {
    // skip version check for now to keep the lkg safety net until release
    // const [major, minor] = cds.version.split('.').map(Number);
    // if (major < 6 || major === 6 && minor < 5) throw new Error('@sap/cds-mtxs >= 1.5 requires @sap/cds version >= 6.5')
